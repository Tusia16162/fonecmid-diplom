...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ВКМ_СформироватьОсновныеНачисления(Движения.ВКМ_ОсновныеНачисления);
...
// ВКМ Начало доработки
Процедура ВКМ_СформироватьОсновныеНачисления(ДвиженияОсновные)
...
Процедура ВКМ_РасчетНДФЛ()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СУММА(ВКМ_ОсновныеНачисленияДанныеГрафика.ВКМ_Результат) КАК ВКМ_Результат,
	|	ВКМ_ОсновныеНачисленияДанныеГрафика.ВКМ_Сотрудник КАК ВКМ_Сотрудник
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаНДФЛ = Выборка.ВКМ_Результат / 100 * 13; 
